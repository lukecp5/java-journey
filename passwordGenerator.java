...
public class passwordGenerator {
    ...
    private static String generatePassword(int length, boolean includeSymbols) {
        ...
        // NOTE: If 'includeSymbols' flag is true, then special symbol characters are added to the 'characters' string.
        if (includeSymbols) {
            characters += "!@#$%^&*()";
        }
        ...
        // A for loop runs 'length' times, where 'length' is the desired length of the password.
        for (int i = 0; i < length; i++) {

            // A random index is generated within the range of the 'characters' string length.
            int index = random.nextInt(characters.length());

            // Character at the randomly generated index is appended to the 'password' string.
